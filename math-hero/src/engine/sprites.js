...
export class SpriteRenderer {
  ...
  setSceneHeight(height, groundY) {
    // 10 px запасу, щоб плюмаж не заходив під край HUD.
    const band = Math.max(60, groundY - height * HUD_SHARE - 10);
  ...
  _bossForm(boss, phase, heroHeight, groundY) {
    ...
    // Keep the boss inside the arena on short canvases…
    if (groundY > 0) target = Math.min(target, groundY * BOSS_PHASE_CAP[index]);
