...
class GameApp {
        ...
        if (this.heroX >= targetHeroX && this.enemyX <= targetEnemyX) {
          ...
          if (this.currentEnemy) {
            if (this.isBoss && this.currentEnemy.quote) {
              this.mathUI.showBossSpeech(this.currentEnemy.quote, this.enemyX, this.renderer.width);
            } else if (this.currentEnemy.isElite || this.currentEnemy.id?.includes('elite')) {
              this.mathUI.showBossSpeech(randomQuote('elite'), this.enemyX, this.renderer.width);
            } else {
              this.mathUI.showBossSpeech(randomQuote('enemy'), this.enemyX, this.renderer.width);
            }
          }
          this.nextQuestion();
        }
  ...
  updateSpeechPositions() {
    ...
    if (enemyBubble && this.currentEnemy) {
      ...
      // Sprite metrics come from the renderer so the bubble can never drift
      // away from the artwork when animation timings change.
      const groundY = this.renderer.height * 0.72;
      const finalY = groundY - this.enemyTopOffset() - 36;
      enemyBubble.style.top = `${(finalY / this.renderer.height) * 100}%`;
    }
  }
  ...
  playEvolution(event) {
    ...
    this.mathUI.showBossSpeech(event.title, this.enemyX, this.renderer.width);
  ...
  handleCorrectAnswer(result) {
    ...
    this.mathUI.showHeroSpeech(randomQuote('heroCorrect'), this.heroX, this.renderer.width);
  ...
  activateUltimate() {
    ...
    this.mathUI.showHeroSpeech(t('combat.ultimateCry'), this.heroX, this.renderer.width);
  ...
  handleIncorrectAnswer(result) {
    ...
    if (hurt.shielded) {
      ...
      this.mathUI.showHeroSpeech(t('combat.shieldBlocked'), this.heroX, this.renderer.width);
    } else {
      this.heroState = 'hurt';
      this.soundEngine.playEnemyAttack(this.enemyAttackSound());
      this.soundEngine.playHurt();
      this.mathUI.showHeroSpeech(randomQuote('heroError'), this.heroX, this.renderer.width);
    }
    ...
    if (this.isBoss) this.mathUI.showBossSpeech(randomQuote('boss'), this.enemyX, this.renderer.width);
