...
export class GameRenderer {
  ...
  resize() {
    ...
    // Update CSS variable so UI overlays (like speech bubbles) can align with ground
    if (typeof document !== 'undefined') {
      document.documentElement.style.setProperty('--ground-y', `${this.groundY}px`);
      document.documentElement.style.setProperty('--scene-scale', `${this.sprites.scale}`);
    }
  }
  ...
  render(gameState, animTime) {
    ...
    if (gameState.hero) {
      this.sprites.drawHero(ctx, gameState.heroX || this.width * 0.23, groundY + 5,
        gameState.hero, gameState.heroState || 'idle', animTime);
    }
    if (gameState.currentEnemy) {
      this.drawCombatant(ctx, groundY + 5, gameState.currentEnemy, gameState.enemyX,
        gameState.isBoss, gameState.enemyState || 'idle', animTime, gameState);
    }
    if (gameState.dying?.enemy) {
      this.drawCombatant(ctx, groundY + 5, gameState.dying.enemy, gameState.dying.x,
  ...
  drawBackground(ctx) {
    ...
    // We want the width to cover the screen width
    const scaleW = this.width / img.naturalWidth;
    
    // We also want the image to reach from the top down to the bottom of the screen.
    // The image's grass line (groundY) is at 72% of its height.
    // So the dirt part of the image is the remaining 28% (from 72% to 100%).
    // We must ensure this 28% covers the space from groundY to the bottom of the screen.
    const requiredDirtHeight = this.height - this.groundY;
    const imageDirtHeight = img.naturalHeight * 0.28;
    const scaleH = requiredDirtHeight > 0 ? requiredDirtHeight / imageDirtHeight : 0;
    
    const scale = Math.max(scaleW, scaleH);
    const drawWidth = img.naturalWidth * scale;
    const drawHeight = img.naturalHeight * scale;
    const drawX = (this.width - drawWidth) / 2; // Center horizontally if scaleH > scaleW
    const drawY = this.groundY - (0.72 * drawHeight);
  ...
  drawMidground(ctx, groundY) {
    ...
    const drawHeight = Math.round(groundY * 0.55); // Scales based on groundY
    const drawWidth = Math.round(drawHeight * aspect);
    const stepX = Math.round(drawWidth * 0.82);
    const drawY = groundY - drawHeight + 6; // Sink slightly behind grass edge
